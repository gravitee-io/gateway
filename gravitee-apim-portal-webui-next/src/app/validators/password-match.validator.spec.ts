...
describe('passwordMatchValidator', () => {
  it('should return an error when passwords do not match', () => {
    ...
    expect(passwordMatchValidator(formGroup.controls.confirmedPassword)).toEqual({ passwordMismatch: true });
  });
  ...
  `('should not return an error when passwords are falsy or match', ({ password, confirmedPassword }) => {
    ...
    expect(passwordMatchValidator(formGroup.controls.confirmedPassword)).toBeNull();
  });
});
